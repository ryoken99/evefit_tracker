...
param(
  ...
  [string]$BaselineApk,
  [string]$ExpectedBaselineVersionName = '1.1.5',
  [int]$ExpectedBaselineVersionCode = 7,
  [string]$CurrentApk,
  [string]$DeviceId,
...
function Get-UiXml([string]$Adb, [string]$TargetDevice) {
  $remote = '/sdcard/evefit_v116_upgrade_window.xml'
...
function Restore-DatabaseToDevice {
  ...
  foreach ($suffix in @('', '-wal', '-shm')) {
    ...
    if (Test-Path -LiteralPath $local) {
      $staged = "/data/local/tmp/evefit_v115_upgrade.db$suffix"
...
def snapshot(connection):
    ...
    return {
        ...
        'markers': {
            'profile': one(connection, "SELECT 1 FROM profiles WHERE notes = 'v115-upgrade-profile'"),
            'measurement': one(connection, "SELECT 1 FROM body_measurements WHERE notes = 'v115-upgrade-measurement'"),
            'goal': one(connection, "SELECT 1 FROM goals WHERE title = 'Upgrade v1.1.5 goal'"),
            'workout': one(connection, "SELECT 1 FROM workouts WHERE notes = 'v115-upgrade-workout'"),
            'workout_exercise': one(connection, "SELECT 1 FROM workout_exercises WHERE notes = 'v115-upgrade-workout-exercise'"),
            'workout_set': one(connection, "SELECT 1 FROM workout_sets WHERE notes = 'v115-upgrade-workout-set'"),
            'settings': one(connection, "SELECT 1 FROM dashboard_widgets WHERE metric_key = 'v115-upgrade-setting' AND title = 'Upgrade setting' AND is_visible = 0 AND sort_order = 77"),
            'historical_join': one(connection, "SELECT 1 FROM workouts w JOIN workout_exercises we ON we.workout_id = w.id JOIN workout_sets ws ON ws.workout_id = w.id AND ws.exercise_id = we.exercise_id JOIN exercises e ON e.id = we.exercise_id WHERE w.notes = 'v115-upgrade-workout' AND we.notes = 'v115-upgrade-workout-exercise' AND ws.notes = 'v115-upgrade-workout-set' AND e.notes = 'v115-upgrade-historical-exercise'"),
        },
    }
...
def prepare(connection):
    ...
    connection.execute(
        ...
        ('Upgrade v1.1.5', pin_hash, now, now, 1, 180.0, 'home', 'preserve-upgrade-settings', 'v115-upgrade-profile'),
    )
    ...
    connection.execute(
        ...
        ('Upgrade v1.1.5 historical exercise', 'Outro', 0, 'v115-upgrade-historical-exercise'),
    )
    ...
    connection.execute(
        ...
        (profile_id, workout_id, exercise_id, 'v115-upgrade-workout-exercise'),
    )
    connection.execute(
        'INSERT INTO workout_sets(profile_id,workout_id,exercise_id,set_number,reps,notes) VALUES(?,?,?,?,?,?)',
        (profile_id, workout_id, exercise_id, 1, 12, 'v115-upgrade-workout-set'),
    )
    connection.execute(
        ...
        (profile_id, 'v115-upgrade-setting', 'Upgrade setting', 0, 77, now, now, now),
    )
    connection.commit()
...
try {
  ...
  if ($baselineMetadata.Package -ne $packageId -or $baselineMetadata.VersionName -ne $ExpectedBaselineVersionName -or $baselineMetadata.VersionCode -ne $ExpectedBaselineVersionCode) {
    throw "The baseline APK is not EveFit $ExpectedBaselineVersionName build $ExpectedBaselineVersionCode."
  }
  if ($currentMetadata.Package -ne $packageId -or $currentMetadata.VersionName -ne '1.1.6' -or $currentMetadata.VersionCode -ne 8) {
    throw 'The current APK is not EveFit v1.1.6 build 8.'
  }
  ...
  if ($baselineSignature.Digest -ne $expectedCertificate -or $currentSignature.Digest -ne $expectedCertificate -or $baselineSignature.Digest -ne $currentSignature.Digest) {
    throw 'SIGNING_INCOMPATIBILITY: the baseline and v1.1.6 certificates do not match the required certificate.'
  }
  ...
  $runDirectory = Join-Path $repository "test_artifacts\release\v1.1.6\upgrade\$ExpectedBaselineVersionName-$timestamp"
  ...
  if ($databaseSize -notmatch '^\d+$' -or [int64]$databaseSize -lt 100000) {
    throw 'The baseline APK did not finish creating its database.'
  }
  ...
  if ($LASTEXITCODE -ne 0) { throw 'Failed to prepare the representative baseline database.' }
  ...
  if ($LASTEXITCODE -ne 0) { throw 'adb install -r failed for the v1.1.6 APK.' }
  ...
  $continue = Wait-UiNode $adb $DeviceId 'Tocar para continuar' '' 20
  if (-not $continue) { throw 'The EFT landing action was not visible after upgrade.' }
  Save-Screenshot $adb $DeviceId (Join-Path $screenshots '00_eft_landing_after_upgrade.png')
  Invoke-UiTap $adb $DeviceId $continue

  $profileChoice = Wait-UiNode $adb $DeviceId 'Upgrade v1.1.5' '' 10
  ...
  $workout = Wait-UiNode $adb $DeviceId 'Treino upgrade v1.1.5' ''
  ...
  $mainTraining = Find-UiNodeWithScroll $adb $DeviceId 'Treino principal'
  ...
  $speedPower = Find-UiNodeWithScroll $adb $DeviceId 'Velocidade e pot'
  if (-not $speedPower) { throw 'The speed-power capability was not visible after upgrade.' }
  Invoke-UiTap $adb $DeviceId $speedPower
  ...
  if (Wait-UiNode $adb $DeviceId 'Upgrade v1.1.5 historical exercise' '' 3) {
    throw 'LEGACY_VISIBLE: the representative legacy exercise is visible in the canonical selector.'
  }
  Scroll-UiListToTop $adb $DeviceId
  $concept = Find-UiNodeWithScroll $adb $DeviceId 'Acelera'
  if (-not $concept) { throw 'Explosive acceleration was not visible after upgrade.' }
  Invoke-UiTap $adb $DeviceId $concept
  if (-not (Wait-UiNode $adb $DeviceId 'Que inten' '' 20)) {
    throw 'The training intention step was not visible after concept selection.'
  }
  $intention = Find-UiNodeWithScroll $adb $DeviceId 'Aumentar a acelera'
  if (-not $intention) { throw 'A real compatible training intention was not visible after upgrade.' }
  Invoke-UiTap $adb $DeviceId $intention
  if (-not (Wait-UiNode $adb $DeviceId 'Detalhe da inten' '' 20)) {
    throw 'The selected training intention detail was not visible after upgrade.'
  }
  $confirmIntention = Wait-UiNode $adb $DeviceId 'Selecionar esta inten' '' 10
  if (-not $confirmIntention) { throw 'The explicit training intention confirmation action was not visible.' }
  Invoke-UiTap $adb $DeviceId $confirmIntention
  if (-not (Wait-UiNode $adb $DeviceId 'Exerc' '' 20)) {
    throw 'The approved Wave1 exercise results were not visible after intention confirmation.'
  }
  $wave1Exercise = Find-UiNodeWithScroll $adb $DeviceId 'Sprint resistido com tren'
  if (-not $wave1Exercise) {
    throw 'The approved high-risk Wave1 variant was not visible after upgrade.'
  }
  if (-not (Wait-UiNode $adb $DeviceId 'Treino principal' '' 10)) {
    throw 'The selected context was not preserved in the complete breadcrumb.'
  }
  Invoke-UiTap $adb $DeviceId $wave1Exercise
  if (-not (Find-UiNodeWithScroll $adb $DeviceId 'Evid' 20)) {
    throw 'The Wave1 exercise detail did not open after upgrade.'
  }
  Save-Screenshot $adb $DeviceId (Join-Path $screenshots '03_training_intentions_after_upgrade.png')
  ...
  if ($LASTEXITCODE -ne 0) { throw 'Failed to verify the upgraded v1.1.6 database.' }
  ...
  foreach ($table in @('profiles', 'body_measurements', 'goals', 'workouts', 'workout_exercises', 'workout_sets', 'dashboard_widgets')) {
    ...
    "EVEFIT_V116_UPGRADE_BEFORE_$($table.ToUpper())=$beforeCount"
    "EVEFIT_V116_UPGRADE_AFTER_$($table.ToUpper())=$afterCount"
  }
  if (@($after.foreign_keys).Count -ne 0) { throw 'Foreign key verification failed.' }
  if ($before.user_version -ne 22 -or $after.user_version -ne 22) {
    throw "SCHEMA_OR_MIGRATION_CHANGE: expected user_version 22 before and after upgrade, found $($before.user_version) and $($after.user_version)."
  }
  ...
  "EVEFIT_V116_BASELINE_VERSION=$($baselineMetadata.VersionName)+$($baselineMetadata.VersionCode)"
  "EVEFIT_V116_CURRENT_VERSION=$($currentMetadata.VersionName)+$($currentMetadata.VersionCode)"
  "EVEFIT_V116_CERTIFICATE_SUBJECT=$($currentSignature.Subject)"
  "EVEFIT_V116_CERTIFICATE_SHA256=$($currentSignature.Digest)"
  'EVEFIT_V116_APK_SIGNATURE_SCHEME_V2=true'
  'EVEFIT_V116_FOREIGN_KEYS_VALID=true'
  'EVEFIT_V116_SCHEMA_VERSION_22=true'
  'EVEFIT_V116_SCHEMA_UNCHANGED=true'
  'EVEFIT_V116_PERSONAL_DATA_PRESERVED=true'
  'EVEFIT_V116_SETTINGS_PRESERVED=true'
  'EVEFIT_V116_HISTORICAL_DATA_ACCESSIBLE=true'
  'EVEFIT_V116_SEVEN_CONTEXTS_VISIBLE=true'
  'EVEFIT_V116_EXPLICIT_INTENTION_CONFIRMATION=true'
  'EVEFIT_V116_WAVE1_RESULT_VISIBLE=true'
  'EVEFIT_V116_LEGACY_VISIBLE=false'
