...
try {
  ...
  if ($LASTEXITCODE -ne 0) { throw 'Failed to stop the app before the selector test.' }

  $timestamp = Get-EveFitTimestamp
  $runDirectory = Join-Path $repository "test_artifacts\workout_exercise_selector_roots\full_app\$timestamp"
  ...
  if ($LASTEXITCODE -ne 0) { throw 'Failed to clear logcat before the selector test.' }
  ...
  try {
    ...
    $command = 'call "{0}" test "integration_test/workout_exercise_selector_roots_full_app_test.dart" -d {1} -r expanded --no-uninstall' -f $flutter, $DeviceId
    ...
    $screenshotLines = Select-String -LiteralPath $stdout -Pattern '^WORKOUT_EXERCISE_SELECTOR_SCREENSHOT=([^|]+)\|(.+)$'
    foreach ($line in $screenshotLines) {
      ...
      if (
        ...
        throw "Failed to capture selector screenshot '$name'."
      }
    }
    & $adb -s $DeviceId shell am force-stop (Get-EveFitPackageId)
    if ($LASTEXITCODE -ne 0) { throw 'Failed to stop the app after the selector test.' }
  } finally {
