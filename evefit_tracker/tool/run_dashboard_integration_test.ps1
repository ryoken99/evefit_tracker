...
try {
  ...
  $runDirectory = New-EveFitRunDirectory 'integration' 'dashboard'
  ...
  $logcatError = Join-Path $runDirectory 'logcat.stderr.log'
  $metadata = Join-Path $runDirectory 'metadata.json'
  Write-EveFitMetadata -Path $metadata -DeviceId $DeviceId

  & $adb -s $DeviceId logcat -c
  if ($LASTEXITCODE -ne 0) { throw 'Failed to clear logcat before the integration test.' }
  $logcatProcess = Start-Process -FilePath $adb -ArgumentList @('-s', $DeviceId, 'logcat') -RedirectStandardOutput $logcat -RedirectStandardError $logcatError -PassThru -WindowStyle Hidden
  ...
  try {
    ...
    $env:EVEFIT_SCREENSHOT_PREFIX = Split-Path $runDirectory -Leaf
  ...
  "DEVICE_ID=$DeviceId"
  "INTEGRATION_RUN_DIRECTORY=$runDirectory"
  "INTEGRATION_LOG=$combined"
  "LOGCAT=$logcat"
  "SCREENSHOT_DIRECTORY=$screenshotDirectory"
  "METADATA=$metadata"
  "INTEGRATION_EXIT_CODE=$result"
  exit $result
} catch {
  if ($logcatProcess -and -not $logcatProcess.HasExited) {
    Stop-Process -Id $logcatProcess.Id -Force
