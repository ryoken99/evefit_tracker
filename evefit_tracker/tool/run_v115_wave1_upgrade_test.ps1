...
param(
  ...
  [string]$BaselineApk,
  [string]$ExpectedBaselineVersionName = '1.1.3',
  [int]$ExpectedBaselineVersionCode = 5,
  [string]$CurrentApk,
  [string]$DeviceId,
...
function Get-UiXml([string]$Adb, [string]$TargetDevice) {
  $remote = '/sdcard/evefit_v115_upgrade_window.xml'
...
function Restore-DatabaseToDevice {
  ...
  foreach ($suffix in @('', '-wal', '-shm')) {
    ...
    if (Test-Path -LiteralPath $local) {
      $staged = "/data/local/tmp/evefit_v114_upgrade.db$suffix"
...
def snapshot(connection):
    ...
    return {
        ...
        'markers': {
            'profile': one(connection, "SELECT 1 FROM profiles WHERE notes = 'v114-upgrade-profile'"),
            'measurement': one(connection, "SELECT 1 FROM body_measurements WHERE notes = 'v114-upgrade-measurement'"),
            'goal': one(connection, "SELECT 1 FROM goals WHERE title = 'Upgrade v1.1.3 goal'"),
            'workout': one(connection, "SELECT 1 FROM workouts WHERE notes = 'v114-upgrade-workout'"),
            'workout_exercise': one(connection, "SELECT 1 FROM workout_exercises WHERE notes = 'v114-upgrade-workout-exercise'"),
            'workout_set': one(connection, "SELECT 1 FROM workout_sets WHERE notes = 'v114-upgrade-workout-set'"),
            'settings': one(connection, "SELECT 1 FROM dashboard_widgets WHERE metric_key = 'v114-upgrade-setting' AND title = 'Upgrade setting' AND is_visible = 0 AND sort_order = 77"),
            'historical_join': one(connection, "SELECT 1 FROM workouts w JOIN workout_exercises we ON we.workout_id = w.id JOIN workout_sets ws ON ws.workout_id = w.id AND ws.exercise_id = we.exercise_id JOIN exercises e ON e.id = we.exercise_id WHERE w.notes = 'v114-upgrade-workout' AND we.notes = 'v114-upgrade-workout-exercise' AND ws.notes = 'v114-upgrade-workout-set' AND e.notes = 'v114-upgrade-historical-exercise'"),
        },
    }
...
def prepare(connection):
    ...
    connection.execute(
        ...
        ('Upgrade v1.1.3', pin_hash, now, now, 1, 180.0, 'home', 'preserve-upgrade-settings', 'v114-upgrade-profile'),
    )
    ...
    connection.execute(
        ...
        ('Upgrade v1.1.3 historical exercise', 'Outro', 0, 'v114-upgrade-historical-exercise'),
    )
    ...
    connection.execute(
        ...
        (profile_id, workout_id, exercise_id, 'v114-upgrade-workout-exercise'),
    )
    connection.execute(
        'INSERT INTO workout_sets(profile_id,workout_id,exercise_id,set_number,reps,notes) VALUES(?,?,?,?,?,?)',
        (profile_id, workout_id, exercise_id, 1, 12, 'v114-upgrade-workout-set'),
    )
    connection.execute(
        ...
        (profile_id, 'v114-upgrade-setting', 'Upgrade setting', 0, 77, now, now, now),
    )
    connection.commit()
...
try {
  ...
  if ($currentMetadata.Package -ne $packageId -or $currentMetadata.VersionName -ne '1.1.5' -or $currentMetadata.VersionCode -ne 7) {
    throw 'The current APK is not EveFit v1.1.5 build 7.'
  }
  ...
  if ($baselineSignature.Digest -ne $expectedCertificate -or $currentSignature.Digest -ne $expectedCertificate -or $baselineSignature.Digest -ne $currentSignature.Digest) {
    throw 'SIGNING_INCOMPATIBILITY: the baseline and v1.1.5 certificates do not match the required certificate.'
  }
  ...
  $runDirectory = Join-Path $repository "test_artifacts\release\v1.1.5\upgrade\$ExpectedBaselineVersionName-$timestamp"
  ...
  if ($LASTEXITCODE -ne 0) { throw 'adb install -r failed for the v1.1.5 APK.' }
  ...
  $profileChoice = Wait-UiNode $adb $DeviceId 'Upgrade v1.1.3' '' 10
  ...
  $workout = Wait-UiNode $adb $DeviceId 'Treino upgrade v1.1.3' ''
  ...
  if (Wait-UiNode $adb $DeviceId 'Upgrade v1.1.3 historical exercise' '' 3) {
  ...
  if ($LASTEXITCODE -ne 0) { throw 'Failed to verify the upgraded v1.1.5 database.' }
  ...
  foreach ($table in @('profiles', 'body_measurements', 'goals', 'workouts', 'workout_exercises', 'workout_sets', 'dashboard_widgets')) {
    ...
    "EVEFIT_V115_UPGRADE_BEFORE_$($table.ToUpper())=$beforeCount"
    "EVEFIT_V115_UPGRADE_AFTER_$($table.ToUpper())=$afterCount"
  }
  ...
  "EVEFIT_V115_BASELINE_VERSION=$($baselineMetadata.VersionName)+$($baselineMetadata.VersionCode)"
  "EVEFIT_V115_CURRENT_VERSION=$($currentMetadata.VersionName)+$($currentMetadata.VersionCode)"
  "EVEFIT_V115_CERTIFICATE_SUBJECT=$($currentSignature.Subject)"
  "EVEFIT_V115_CERTIFICATE_SHA256=$($currentSignature.Digest)"
  'EVEFIT_V115_APK_SIGNATURE_SCHEME_V2=true'
  'EVEFIT_V115_FOREIGN_KEYS_VALID=true'
  'EVEFIT_V115_SCHEMA_VERSION_22=true'
  'EVEFIT_V115_SCHEMA_UNCHANGED=true'
  'EVEFIT_V115_PERSONAL_DATA_PRESERVED=true'
  'EVEFIT_V115_SETTINGS_PRESERVED=true'
  'EVEFIT_V115_HISTORICAL_DATA_ACCESSIBLE=true'
  'EVEFIT_V115_SEVEN_CONTEXTS_VISIBLE=true'
  'EVEFIT_V115_EXPLICIT_INTENTION_CONFIRMATION=true'
  'EVEFIT_V115_WAVE1_RESULT_VISIBLE=true'
  'EVEFIT_V115_LEGACY_VISIBLE=false'
