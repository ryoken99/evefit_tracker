...
try {
  ...
  # Do not let a manually pre-warmed app instance keep the test driver from
  # launching the instrumented process for this run.
  & $adb -s $DeviceId shell am force-stop (Get-EveFitPackageId)
  if ($LASTEXITCODE -ne 0) { throw 'Failed to stop the app before the canonical search test.' }

  $timestamp = Get-EveFitTimestamp
  $runDirectory = Join-Path $repository "test_artifacts\canonical_search_menu\integration\$timestamp"
  ...
  if ($LASTEXITCODE -ne 0) { throw 'Failed to clear logcat before the canonical search test.' }
  ...
  try {
    ...
    $command = 'call "{0}" drive --driver "test_driver/integration_test.dart" --target "integration_test/canonical_search_menu_flow_test.dart" -d {1}' -f $flutter, $DeviceId
  ...
  "DEVICE_ID=$DeviceId"
  "CANONICAL_SEARCH_RUN_DIRECTORY=$runDirectory"
  "CANONICAL_SEARCH_LOG=$combined"
  "LOGCAT=$logcat"
  "SCREENSHOT_DIRECTORY=$screenshotDirectory"
  "METADATA=$metadata"
  "CANONICAL_SEARCH_EXIT_CODE=$result"
  exit $result
} catch {
  if ($logcatProcess -and -not $logcatProcess.HasExited) {
    Stop-Process -Id $logcatProcess.Id -Force
