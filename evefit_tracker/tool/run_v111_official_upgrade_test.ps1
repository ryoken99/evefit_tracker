...
function Get-UiXml([string]$Adb, [string]$TargetDevice) {
  $remote = '/sdcard/evefit_v111_upgrade_window.xml'
  ...
  do {
    ...
    if ($xml) {
      $node = @($xml.SelectNodes('//node')) | Where-Object {
        (-not $Text -or $_.text -eq $Text -or $_.'content-desc' -eq $Text -or $_.'content-desc' -like "$Text*") -and
...
function Restore-DatabaseToDevice {
  ...
  foreach ($suffix in @('', '-wal', '-shm')) {
    ...
    if (Test-Path -LiteralPath $local) {
      $staged = "/data/local/tmp/evefit_v111_upgrade.db$suffix"
...
try {
  ...
  if ($baselineMetadata.Package -ne $packageId -or $baselineMetadata.VersionName -ne '1.1.0' -or $baselineMetadata.VersionCode -ne 2) {
    throw 'The baseline APK is not the official EveFit v1.1.0 build 2 package.'
  }
  if ($currentMetadata.Package -ne $packageId -or $currentMetadata.VersionName -ne '1.1.1' -or $currentMetadata.VersionCode -le $baselineMetadata.VersionCode) {
    throw 'The current APK does not have the expected monotonic v1.1.1 metadata.'
  }
  ...
  if ($baselineCertificate -ne $currentCertificate) {
    throw 'SIGNING_INCOMPATIBILITY: the v1.1.0 and v1.1.1 certificates differ.'
  }
  ...
  $runDirectory = Join-Path $repository "test_artifacts\release\v1.1.1\upgrade\$timestamp"
  ...
  if ($LASTEXITCODE -ne 0) { throw 'Failed to install the official v1.1.0 APK.' }
  ...
  if ($databaseSize -notmatch '^\d+$' -or [int64]$databaseSize -lt 100000) {
    throw 'The official v1.1.0 APK did not finish creating its database.'
  }
  & $adb -s $DeviceId shell am force-stop $packageId | Out-Null
  $owner = (& $adb -s $DeviceId shell stat -c '%u' "/data/data/$packageId/databases/evefit_tracker.db").Trim()
  $group = (& $adb -s $DeviceId shell stat -c '%g' "/data/data/$packageId/databases/evefit_tracker.db").Trim()
  ...
  if ($LASTEXITCODE -ne 0) { throw 'Failed to prepare the representative v1.1.0 database.' }
  ...
  if ($LASTEXITCODE -ne 0) { throw 'adb install -r failed for the v1.1.1 APK.' }
  & $adb -s $DeviceId logcat -c | Out-Null
  & $adb -s $DeviceId shell am start -n "$packageId/.MainActivity" | Out-Null
  Start-Sleep -Seconds 8
  $processId = (& $adb -s $DeviceId shell pidof $packageId).Trim()
  ...
  if ($profileChoice) {
    Invoke-UiTap $adb $DeviceId $profileChoice
  }
  ...
  $dashboard = Wait-UiNode $adb $DeviceId 'Dashboard' ''
  if (-not $dashboard) { throw 'The upgraded profile did not reach Dashboard.' }
  ...
  $addExercise = Wait-UiNode $adb $DeviceId 'Adicionar' ''
  ...
  if (-not (Wait-UiNode $adb $DeviceId 'Explorar' '')) {
    throw 'The Canonical Core did not open after upgrade.'
  }
  foreach ($label in @('Por capacidade', 'Por inten', 'Por conceito de treino', 'Por contexto')) {
    if (-not (Wait-UiNode $adb $DeviceId $label '' 10)) {
      throw "Canonical Core axis missing after upgrade: $label"
    }
  }
  $xml = Get-UiXml $adb $DeviceId
  $xmlText = $xml.OuterXml
  if ($xmlText -match 'Mostrar todos|Parte superior') {
    throw 'An unapproved or legacy search surface is visible after upgrade.'
  }
  Save-Screenshot $adb $DeviceId (Join-Path $screenshots '03_canonical_core_after_upgrade.png')
  ...
  if ($LASTEXITCODE -ne 0) { throw 'Failed to verify the upgraded v1.1.1 database.' }
  ...
  foreach ($table in @('profiles', 'body_measurements', 'goals', 'workouts', 'workout_sets', 'workout_exercises', 'exercises')) {
    ...
    "EVEFIT_V111_UPGRADE_BEFORE_$($table.ToUpper())=$beforeCount"
    "EVEFIT_V111_UPGRADE_AFTER_$($table.ToUpper())=$afterCount"
  }
  ...
  "EVEFIT_V111_BASELINE_VERSION=$($baselineMetadata.VersionName)+$($baselineMetadata.VersionCode)"
  "EVEFIT_V111_CURRENT_VERSION=$($currentMetadata.VersionName)+$($currentMetadata.VersionCode)"
  "EVEFIT_V111_CERTIFICATE_SHA256=$currentCertificate"
  'EVEFIT_V111_FOREIGN_KEYS_VALID=true'
  'EVEFIT_V111_PERSONAL_DATA_PRESERVED=true'
  'EVEFIT_V111_CANONICAL_CORE_VISIBLE=true'
  'EVEFIT_V111_LEGACY_VISIBLE=false'
