...
Future<void> main(List<String> argv) async {
  ...
  try {
    final jsonReportPath = _optionValue(argv, '--json-report');
    if (jsonReportPath != null) {
      jsonReportFile = safeJsonReportFile(root, jsonReportPath);
    }
    if (argv.isEmpty) {
    ...
    if (command == 'manifest') {
      ...
      final path =
          _optionValue(argv, '--input') ?? _optionValue(argv, '--profile');
      ...
      final resolution = await resolveChangedFiles(argv, root);
      report['changeDiscovery'] = resolution.toJson();
      if (!resolution.succeeded) {
        exitCode = resolution.exitCode;
        report['reason'] = resolution.reason;
      } else {
        final changed = resolution.files;
        ...
        final options = GateOptions(
          ...
          baselineApk: _optionValue(argv, '--baseline-apk'),
          currentApk: _optionValue(argv, '--current-apk'),
        );
  ...
  var encoded = encodeJson(report);
  if (jsonReportFile != null) {
    try {
      jsonReportFile.parent.createSync(recursive: true);
      jsonReportFile.writeAsStringSync('$encoded\n');
    } on FileSystemException catch (error) {
      exitCode = exitEnvironment;
      report['error'] = 'Failed to write JSON report: ${error.message}';
      report['exitCode'] = exitCode;
      report['result'] = 'fail';
      encoded = encodeJson(report);
    }
  }
...
String? _optionValue(List<String> args, String flag) {
  ...
  if (index + 1 >= args.length || args[index + 1].startsWith('--')) {
    throw _Usage('$flag requires a value');
  }
  if (args.indexOf(flag, index + 1) >= 0) {
    throw _Usage('$flag may be supplied only once');
  }
  return args[index + 1];
}
