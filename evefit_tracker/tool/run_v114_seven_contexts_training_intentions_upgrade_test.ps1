...
function Get-UiXml([string]$Adb, [string]$TargetDevice) {
  $remote = '/sdcard/evefit_v114_upgrade_window.xml'
...
try {
  ...
  if ($baselineMetadata.Package -ne $packageId -or $baselineMetadata.VersionName -ne '1.1.3' -or $baselineMetadata.VersionCode -ne 5) {
    throw 'The baseline APK is not EveFit v1.1.3 build 5.'
  }
  if ($currentMetadata.Package -ne $packageId -or $currentMetadata.VersionName -ne '1.1.4' -or $currentMetadata.VersionCode -ne 6) {
    throw 'The current APK is not EveFit v1.1.4 build 6.'
  }
  ...
  if ($baselineSignature.Digest -ne $expectedCertificate -or $currentSignature.Digest -ne $expectedCertificate -or $baselineSignature.Digest -ne $currentSignature.Digest) {
    throw 'SIGNING_INCOMPATIBILITY: the v1.1.3 and v1.1.4 certificates do not match the required certificate.'
  }
  ...
  $runDirectory = Join-Path $repository "test_artifacts\release\v1.1.4\upgrade\$timestamp"
  ...
  if ($LASTEXITCODE -ne 0) { throw 'Failed to install the v1.1.3 APK.' }
  ...
  if ($databaseSize -notmatch '^\d+$' -or [int64]$databaseSize -lt 100000) {
    throw 'The v1.1.3 APK did not finish creating its database.'
  }
  ...
  if ($LASTEXITCODE -ne 0) { throw 'Failed to prepare the representative v1.1.3 database.' }
  ...
  if ($LASTEXITCODE -ne 0) { throw 'adb install -r failed for the v1.1.4 APK.' }
  ...
  if (-not $warmup) { throw 'The warmup context was not visible after upgrade.' }
  ...
  $intention = Find-UiNodeWithScroll $adb $DeviceId 'Aumentar a temperatura corporal global'
  ...
  if (-not (Wait-UiNode $adb $DeviceId 'Ainda n' '' 20)) {
    throw 'The approved empty exercise state was not visible after intention confirmation.'
  }
  if (-not (Wait-UiNode $adb $DeviceId 'Os exerc' '' 10)) {
    throw 'The empty exercise-state explanation was not visible after intention confirmation.'
  }
  if (-not (Wait-UiNode $adb $DeviceId 'Aquecimento' '' 10)) {
  ...
  if ($LASTEXITCODE -ne 0) { throw 'Failed to verify the upgraded v1.1.4 database.' }
  ...
  foreach ($table in @('profiles', 'body_measurements', 'goals', 'workouts', 'workout_exercises', 'workout_sets', 'dashboard_widgets')) {
    ...
    "EVEFIT_V114_UPGRADE_BEFORE_$($table.ToUpper())=$beforeCount"
    "EVEFIT_V114_UPGRADE_AFTER_$($table.ToUpper())=$afterCount"
  }
  ...
  "EVEFIT_V114_BASELINE_VERSION=$($baselineMetadata.VersionName)+$($baselineMetadata.VersionCode)"
  "EVEFIT_V114_CURRENT_VERSION=$($currentMetadata.VersionName)+$($currentMetadata.VersionCode)"
  "EVEFIT_V114_CERTIFICATE_SUBJECT=$($currentSignature.Subject)"
  "EVEFIT_V114_CERTIFICATE_SHA256=$($currentSignature.Digest)"
  'EVEFIT_V114_APK_SIGNATURE_SCHEME_V2=true'
  'EVEFIT_V114_FOREIGN_KEYS_VALID=true'
  'EVEFIT_V114_SCHEMA_VERSION_22=true'
  'EVEFIT_V114_SCHEMA_UNCHANGED=true'
  'EVEFIT_V114_PERSONAL_DATA_PRESERVED=true'
  'EVEFIT_V114_SETTINGS_PRESERVED=true'
  'EVEFIT_V114_HISTORICAL_DATA_ACCESSIBLE=true'
  'EVEFIT_V114_SEVEN_CONTEXTS_VISIBLE=true'
  'EVEFIT_V114_EXPLICIT_INTENTION_CONFIRMATION=true'
  'EVEFIT_V114_LEGACY_VISIBLE=false'
