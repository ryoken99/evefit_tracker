...
void main() {
  group('v1.0.0 equipment and location compatibility', () {
    test('normalizes aliases and resolves ambiguous tokens by context', () {
      expect(
        EquipmentCatalogService.normalizeEquipmentToken('halteres'),
        'dumbbells',
      );
      expect(
        EquipmentCatalogService.normalizeEquipmentToken('dumbbell'),
        'dumbbells',
      );
      expect(
        EquipmentCatalogService.normalizeEquipmentToken('barra livre'),
        'barbell',
      );
      expect(
        EquipmentCatalogService.normalizeEquipmentToken('barra fixa'),
        'pullup_bar',
      );
      expect(
      ...
      expect(
        EquipmentCatalogService.normalizeEquipmentToken('saco de pancada'),
        'heavy_bag',
      );
      expect(
        EquipmentCatalogService.normalizeEquipmentToken('saco de areia'),
        'sandbag',
      );
      expect(
        EquipmentCatalogService.normalizeEquipmentToken('banco inclinado'),
        'incline_bench',
      );
      expect(
        EquipmentCatalogService.normalizeEquipmentToken('cadeira'),
        'chair_support',
      );
    });
    ...
    test('separates equipment, surfaces and supports', () {
      expect(
        EquipmentCatalogService.kindFor('dumbbells'),
        CapabilityKind.equipment,
      );
      ...
      expect(
        EquipmentCatalogService.kindFor('chair_support'),
        CapabilityKind.support,
      );
      expect(EquipmentCatalogService.kindFor('bench'), CapabilityKind.support);
    });

    test(
      'home without equipment exposes only basic compatible capabilities',
      () {
